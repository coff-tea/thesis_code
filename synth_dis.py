...
Author: Xi Lu
File: synth_dis.py
Description: Used to perform discrimination between positive and synthetic samples.
""" 
...
# Misc. arguments
parser.add_argument("-y", "--yaml", type=str, default="synth_dis", dest="y", required=False, help="Name of yaml file storing relevant information. [DEFAULT: %(default)s]")
...
#--------- Save name
save_spec = "synthdis" 
save_name = f"{args.origin}_{save_spec}_k{args.kth_best}_{args.model_name}"
...
#--------- Get positive data
pos_frames = spec_data.load_data(np.load(f"{data_name_prefix}pos.npy"))
...
#--------- Make dataloaders
train_data = [[np.array(syn_frames[i]), 0] for i in idx_dict["syn"][0]]
...
del train_data
test_data = [[np.array(syn_frames[i]), 0] for i in idx_dict["syn"][1]]
...
del test_data
val_data = [[np.array(syn_frames[i]), 0] for i in idx_dict["syn"][2]]
