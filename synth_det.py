...
Author: Xi Lu
File: synth_det.py
Description: Used to perform detection using negative and synthetic samples.
""" 
...
parser.add_argument("-reuse", action="store_true", help="When incorporating blind data, only add to positive dataset. [DEFAULT: %(default)s]")
# Misc. arguments
parser.add_argument("-y", "--yaml", type=str, default="synth_det", dest="y", required=False, help="Name of yaml file storing relevant information. [DEFAULT: %(default)s]")
...
uniq = 1
save_spec = "synthdet" 
save_name = f"{args.origin}_{save_spec}_k{args.kth_best}-{uniq}_{args.model_name}"
found_flag = True 
while found_flag:
    try:
        ...
        if uniq == 1:
            if args.skip_unn:
                save_spec = f"{save_spec}skip"
            if args.reuse:
                save_spec = f"{save_spec}reuse"
        uniq += 1
        save_name = f"{args.origin}_{save_spec}_k{args.kth_best}-{uniq}_{args.model_name}"
    except:
        found_flag = False
...
if uniq > 1:        # Not the first run, load previous model
    prev_name = f"{args.origin}_synthdet_k{args.kth_best}-{uniq-1}_{args.model_name}"
    if uniq > 2:
        prev_name = f"{args.origin}_{save_spec}_k{args.kth_best}-{uniq-1}_{args.model_name}"
    ...
    pred = find_positives(dataloader)
